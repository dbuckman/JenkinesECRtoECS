          ...
          steps {
            container('awscli') { 
                script {
                    ...
                    def ECR_VERSION = sh(script: "/bin/bash -c '/usr/local/bin/aws ecr describe-images --region ${AWS_REGION} --repository-name ${AWS_ECR_IMAGE} --output text --query \'sort_by(imageDetails,& imagePushedAt)[*].imageTags[*]\' | tr \'\t\' \'\n\' | tail -1'", returnStdout: true)
